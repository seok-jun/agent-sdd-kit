...
| 세션 결과 | stdout JSONL + session rollout | stream-json message |
...
`skill_triggered`는 최종 답변, 파일 경로 또는 raw stdout의 substring을 검색하지 않는다. Harness별 발동 기록만 구조적으로 읽는다.

- Claude는 `tool_use.name == "Skill"`이면서 `input.skill`이 대상 Skill과 일치하는지 확인한다.
- Codex는 `thread.started.thread_id`로 `~/.codex/sessions`의 rollout을 찾고, `role=user` 레코드의 `<skill><name>...</name></skill>`만 확인한다.
- Claude의 `system/init.skills`와 Codex의 `world_state.host_skills`는 설치 목록이므로 발동 증거로 사용하지 않는다.
- 사용자 prompt 자체에 Skill 이름이 있어도 tool event가 없으면 trigger로 판정하지 않는다.

Codex rollout을 찾거나 읽지 못하면 non-trigger 통과로 간주하지 않고 해당 trial을 `error`로 처리한다. with-Skill 실행에 trigger 케이스가 하나 이상 포함됐는데 모든 trial에서 trigger 증거가 0건이면 개별 행동 실패가 아니라 detector 또는 adapter 오류로 보고 run 전체를 `error`로 종료한다. non-trigger 케이스만 선택하면 detector를 검증할 수 없으므로 경고를 출력한다.
...
- Claude prompt 경계와 allow-list 구성, Codex 격리 flag, stdin 차단이 적용되는지
- Claude 구조화 Skill event와 Codex rollout의 user-role Skill record를 구분하는지
- 다중 응답 결합, rate-limit 오류, permission denial 수집이 동작하는지
- fixture baseline, 외부 Skill 참조 복사, 신규 파일 내용을 포함한 diff 수집이 동작하는지
...
`--dry-run`은 fixture와 CLI command만 준비하고 Agent를 호출하지 않는다. CLI가 실제로 prompt를 받는지, trace와 rollout parser가 현재 버전의 event를 읽는지는 검증하지 못하므로 smoke run을 대신하지 않는다.
...
2. 대상 Harness의 프로젝트 Skill 경로에 Skill을 설치한다. `SKILL.md`의 로컬 Markdown 링크가 Skill 폴더 밖 파일을 가리키면 상대 관계를 유지해 함께 복사한다. 링크가 없거나 저장소·workspace 경계를 벗어나면 실행 전에 error로 중단한다.
...
│       ├── response.md
│       ├── workspace.diff
│       ├── codex-rollout.jsonl        # Codex만
│       ├── permission-denials.json    # 발생한 Claude run만
│       ├── rate-limit-events.json     # 발생한 Claude run만
│       └── result.json
...
신규 파일은 `git add --intent-to-add` 후 diff를 수집하므로 `workspace.diff`에 파일명뿐 아니라 실제 내용도 남는다. Codex 발동 판정에 사용한 원본 rollout과 Claude의 permission denial·rate-limit event도 별도 artifact로 보존한다. 실패를 고칠 때는 최종 응답만 보지 말고 trace와 diff를 함께 확인한다.

토큰과 비용은 보조 지표다. 같은 prompt라도 첫 trial이 prompt cache 생성 비용을 부담할 수 있어 trial별 비용을 Skill 품질이나 Harness 성능의 단독 기준으로 비교하지 않는다.
...
Codex rollout parser와 `command_execution` 기반 `inspects_diff` 판정은 합성 event 단위 테스트를 통과했지만, 실제 Skill 발동 run의 현재 `codex --json`과 session rollout 조합에서는 별도로 확인해야 한다. rollout 위치나 schema가 바뀌어 파일을 찾지 못하는 경우 결과를 fail로 낮추지 않고 adapter error로 남긴다.
